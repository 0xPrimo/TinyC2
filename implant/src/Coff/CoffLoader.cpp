...
BOOL COFFCleanup( COFF_CONTEXT* Ctx ) {
    ...
    if (Ctx->GlobalOffsetTable)
        VirtualFree(
            Ctx->GlobalOffsetTable,
            Ctx->FileHeader->NumberOfSymbols * sizeof( PVOID ),
            MEM_RELEASE );
...
DWORD COFFRoutine( COFF_ENTRYPOINT_DATA* Args ) {
...
BOOL COFFRun( COFF_CONTEXT* Ctx, BOOL Sync, PVOID ArgsData, DWORD ArgsSize ) {
    ...
    for (int i = 0; i < Ctx->FileHeader->NumberOfSymbols; i++) {
        if (!strcmp( Ctx->SymbolTable[i].First.Name, "go" )) {
            entry =
                (COFF_ENTRYPOINT)( (DWORD_PTR)
                                       Ctx->SectionBuffer[Ctx->SymbolTable[i].SectionNumber - 1] +
                                   Ctx->SymbolTable[i].Value );
            data = (COFF_ENTRYPOINT_DATA*)HeapAlloc(
                GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof( COFF_ENTRYPOINT_DATA ) );
            if (!data) {
                return FALSE;
            }
            ...
            if (Sync) {
                ...
                hThread =
                    CreateThread( NULL, 0, (LPTHREAD_START_ROUTINE)COFFRoutine, data, 0, NULL );
                if (!hThread) {
                    return FALSE;
                }
...
static BOOL COFFProcessRelocations(
    COFF_CONTEXT* Ctx, DWORD SectionIndex, PCOFF_SECTION_HEADER SectionHeader,
    ...
    for (DWORD RelocationIndex = 0; RelocationIndex < SectionHeader->NumberOfRelocations;
         RelocationIndex++) {
        ...
        if (IS_SYMBOL_EXTERNAL( &Ctx->SymbolTable[Relocation->SymbolTableIndex] )) {
            ...
            SymAddress = Ctx->SectionBuffer
                             [Ctx->SymbolTable[Relocation->SymbolTableIndex].SectionNumber - 1];
            SymAddress = (PVOID)( (DWORD_PTR)SymAddress +
                                  Ctx->SymbolTable[Relocation->SymbolTableIndex].Value );
        } else {
            printf(
                "[-] Relocation %d in section %d references undefined symbol %s\n",
                SectionIndex,
                RelocationIndex,
                SymName );
            return FALSE;
        ...
        for (DWORD Index = 0; Index < 8; Index++) {
            if (COFFRelocationHandlers[Index].Type == Relocation->Type) {
                if (!COFFRelocationHandlers[Index].Handler(
                        Ctx, SectionIndex, Relocation, SymAddress )) {
...
static BOOL COFFProcessSections( COFF_CONTEXT* Ctx ) {
    ...
    for (int i = 0; i < Ctx->FileHeader->NumberOfSections; i++) {
        ...
        SectionRelocations =
            (PCOFF_RELOCATION)( Ctx->FileBuffer + SectionHeader->PointerToRelocations );
        ...
        if (!VirtualProtect(
                ...
                GetSectionProtection( SectionHeader->Characteristics ),
                &OldProtection )) {
...
static BOOL COFFLoadSections( COFF_CONTEXT* Ctx ) {
    PCOFF_SECTION_HEADER SectionHeader = NULL;
...
static BOOL COFFInitialize( COFF_CONTEXT** Ctx, PVOID Data, DWORD Size ) {
    ...
    Context =
        (COFF_CONTEXT*)HeapAlloc( GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof( COFF_CONTEXT ) );
    if (!Context) {
    ...
    Context->SymbolTable =
        (PCOFF_SYMBOL)( (PBYTE)Data + Context->FileHeader->PointerToSymbolTable );
    Context->StringTable = (CHAR*)( Context->SymbolTable + Context->FileHeader->NumberOfSymbols );

    SectionBuffer = (PVOID*)HeapAlloc(
        GetProcessHeap(),
        HEAP_ZERO_MEMORY,
        sizeof( PVOID ) * Context->FileHeader->NumberOfSections );
    if (!SectionBuffer) {
        printf( "HeapAlloc failed: ( %d )\n", GetLastError() );
        goto FAILED;
    }
    ...
    GlobalOffsetTable     = VirtualAlloc(
        NULL, GlobalOffsetTableSize, MEM_COMMIT | MEM_RESERVE | MEM_TOP_DOWN, PAGE_READWRITE );
...
BOOL COFFLoader( BOOL Sync, PVOID FileData, DWORD FileSize, PVOID ArgsData, DWORD ArgsSize ) {
...
static PVOID GetSymbolAddress( CHAR* SymName ) {
    if (!strncmp( SymName, "Beacon", 6 ) || !strcmp( SymName, "toWideChar" ) ||
        !strcmp( SymName, "GetProcAddress" ) || !strcmp( SymName, "GetModuleHandleA" ) ||
        !strcmp( SymName, "LoadLibraryA" ) || !strcmp( SymName, "FreeLibrary" )) {
